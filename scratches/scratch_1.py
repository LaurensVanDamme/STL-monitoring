...
def computeEventually(x):
    ...
    while t + a < x[0][-1]:  # TODO: changed all i+1 to i -> doesn' seem to work
        # if len(M) <= 0:
        #     M += [i + 1]
        ...
        # elif len(M) == 0:
        #     t = x[0][i] - b  # Should never happen
        # else:
        t = min(x[0][min(M)] - a, x[0][i] - b)
        if len(M) > 0 and t == x[0][min(M)] - a:
            M.remove(min(M))
            s = t
        if i < len(x[0]) and t == x[0][i] - b:
            while len(M) != 0 and x[1][x[0].index(x[0][i] - b)] >= x[1][max(M)]:  # x[0][i] - b could be just t
                M.remove(max(M))
            M += [i]
            i += 1
        ...
        if s >= x[0][0] and s != t:  # TODO: check if M is empty and use formula with max (in test.py)
            if len(M) > 0:
                ...
                for t_ in x[0]:
                    ...
                    if t_ >= t + b:
                        break
                yt_minM = max([getAffinePoint(x, t_) for t_ in t_options])
